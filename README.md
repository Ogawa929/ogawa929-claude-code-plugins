...
| `robotic-persona` | A `Robotic` [output style](https://code.claude.com/docs/en/output-styles) (not a skill) that reduces answers to telegraphic output for a PM reader: conclusion first, one fact per line, no copula, no connectives, no evaluative or emotional words. Claude reports as an SE to a PM, so a detail appears when the decision turns on it and is left out otherwise, and every claim is labelled 確認済み / 推測 / 不明 — an unknown is declared, never filled with a plausible guess. Japanese output drops です・ます and unambiguous particles — but accuracy overrides terseness, so anything whose absence would permit two readings comes back. Enable it under `/config` → Output style, then `/clear`. |
