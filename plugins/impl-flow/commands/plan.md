...
1. **Gather the input**
   - If the design phase (`design.md`, via `/impl-flow:spec` or `/impl-flow:all`) or the diagnose phase (`diagnose.md`, via `/impl-flow:fix`) was just run in this same session, use the requirements finalized there. Either one also settled the task-set directory and wrote or revised `{task-set directory}/usecases.md`; treat that document as the authoritative statement of *what behaviour is expected*, and this conversation as the additional context around it. If the user edited the document after it was written, re-read it — their edits win over what was said in the conversation.
   - **If the diagnose phase ran, four of its outputs are inputs here, not things to re-derive**: the task-set directory (step 2), the verification contract (step 3), the set of use cases marked "in scope, unchanged" that still need re-verifying, and the files that its omission review landed findings on. Plan for all of them.
...
2. **Identify or create the task-set directory**
   - If the design phase or the diagnose phase ran in this session, it already settled the task-set directory and wrote or revised the `usecases.md` there. **Use that directory and skip the rest of this step** — do not ask the user to choose again, and do not create a second dated directory for the same work. This matters most on the diagnose path: that directory will usually already have plans in `after/` and look finished, and appending to it is correct rather than a reason to start fresh. Continue the `seq` numbering from the highest existing one (step 5), never restart at `01`.
...
3. **Confirm the verification method**
   ...
   - **On the diagnose path, one part of this answer is already fixed and is not open to a choice**: every plan first adds a test that fails for the reported symptom, and passes only once that test goes green. Ask the user about the rest of the verification policy as usual, but carry that requirement into every plan — where diagnose recorded that the symptom cannot be captured automatically, carry its manual reproduction steps instead.
...
4. **Create the implementation plans (using the Plan agent)**
   ...
   - Do not just hand the task off blindly; make the following constraints explicit in the prompt:
     ...
     - **On the diagnose path, the omission review's result has to survive into the plans.** Every file a layer-2 or layer-3 finding landed on must appear in some plan's `files` list, or be named in that plan's body as deliberately excluded with the reason. A finding that stays in the conversation is a finding that did not get fixed, which defeats the point of having run the review. State this in the prompt, and check it against the returned plans in step 5 before saving them.
