---
description: Execute the pending implementation plans in a task-set directory. The main session acts as leader and dispatches each plan to a sonnet subagent, running independent plans in parallel within a single branch (Phase 3 of impl-flow: design -> plan -> implement)
...
2. **Read the plans and check for dependency cycles**
   ...
   - Parse each plan's structured header (`depends_on`, `files`) — do not rely on guessing these from prose.
...
3. **Design the execution batches**
   ...
   - Report the batching result (which plan is in which batch, parallel or sequential, and why) to the user as an informational summary, then proceed — this is not a confirmation gate; the design/plan phases already established what should be built, so do not block execution waiting for approval here.
...
4. **Execute each batch**
   ...
   - Call the Agent tool once per plan, all within a single message (so they run concurrently), each with `model: "sonnet"`. Pass the full text of the corresponding plan file as the prompt, and state explicitly:
   ...
   - Call the Agent tool once, with `model: "sonnet"`. Give it the same instructions as above (stay within declared `files`, do not commit, report changed files + suggested message).
